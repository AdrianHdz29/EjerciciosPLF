...
un_solo_director = list(filter(lambda director: director["Director"] == "Christopher Nolan", peliculas))
sus_peliculas = list(map(lambda pelicula: {"titulo": pelicula["Titulo"], "calificacion": pelicula["CalificaciónIMDB"]}, un_solo_director))

promedio_calificacion = reduce(lambda aux, calificacion: aux + float(calificacion["calificacion"]), sus_peliculas, 0)

print(json.dumps(un_solo_director, indent=2))
print(json.dumps(sus_peliculas, indent=2))
print("Promedio de las calificaciones:   ", promedio_calificacion)